...
outDir = "/d4/karsten/NWM/Short"
ftpDir = "pub/data/nccf/com/nwm/prod"
tmpDir = "/home/karsten/tmp"

pid = os.getpid()
lockFile = tmpDir + "/get_NWM_Short.lock"
...
def errOut(msgContent,emailTitle,emailRec):
	...
	s.quit()
	# Remove lock file
	lockFile = '/home/karsten/tmp/get_NWM_Short.lock'
	os.remove(lockFile)
	sys.exit(1)
...
errTitle = 'Error_pull_NWM_Short'
warningTitle = 'Warning_pull_NWM_Short'
...
if os.path.isfile(lockFile):
	...
	warningMsg =  "WARNING: Another Short Pull Program Running. PID: " + pid
...
# Loop through previous Short cycles, pull each hourly Short cycle,
# read in SWE/Snow Depth data, and store it as short integers
# in output NetCDF format.
...
for hoursBack in range(numHoursBack,hoursPad,-1):
	...
	# Loop through 15 forecasted hours of Short range cycle, download and
   # process data.
	for fHour in range(1,16):
		...
		fileDownload = "nwm.t" + dCycle.strftime('%H') + \
     		            "z.short_range.land.f" + fHourStr + ".conus.nc.gz"
		fileOut = outDirFinal + "/NWM_SNOW_LSM_SHORT_" + dCycle.strftime('%Y%m%d%H') + "_F" + fHourStr + ".nc"
		fileTmp = "nwm.t" + dCycle.strftime('%H') + "z.short_range.land.f" + \
					 fHourStr + ".conus.nc"
		...
		if os.path.isfile(fileOut):
			...
			ftpDir = "pub/data/nccf/com/nwm/prod/nwm." + dCycle.strftime('%Y%m%d') + "/short_range"
