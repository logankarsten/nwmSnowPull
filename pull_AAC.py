...
outDir = "/d4/karsten/NWM/AAC"
ftpDir = "pub/data/nccf/com/nwm/prod"
tmpDir = "/home/karsten/tmp"

pid = os.getpid()
lockFile = tmpDir + "/get_NWM_AAC.lock"
...
def errOut(msgContent,emailTitle,emailRec):
	...
	s.quit()
	# Remove lock file
	lockFile = '/home/karsten/tmp/get_NWM_AAC.lock'
	os.remove(lockFile)
	sys.exit(1)
...
errTitle = 'Error_pull_NWM_AAC'
warningTitle = 'Warning_pull_NWM_AAC'
...
if os.path.isfile(lockFile):
	...
	warningMsg =  "WARNING: Another AAC Pull Program Running. PID: " + pid
...
# Loop through previous AAC cycles, pull each hourly AAC cycle,
...
for hoursBack in range(numHoursBack,hoursPad,-1):
	...
	# Pull AAC file for hour
	fileDownload = "nwm.t" + dCycle.strftime('%H') + \
     	            "z.analysis_assim.land.tm00.conus.nc.gz"
	fileOut = outDirFinal + "/NWM_SNOW_LSM_AAC_" + dCycle.strftime('%Y%m%d%H') + "_F00.nc"
	fileTmp = "nwm.t" + dCycle.strftime('%H') + "z.analysis_assim.land.tm00.conus.nc"		
	...
	if os.path.isfile(fileOut):
		...
		ftpDir = "pub/data/nccf/com/nwm/prod/nwm." + dCycle.strftime('%Y%m%d') + "/analysis_assim"
